# show speed in km/h
# Record MP4, log report of Collision Data as CSV file
# Horn added
import carla
import pygame
import sys
...
def on_collision(event):
    ...
    impact_point = event.normal_impulse
    timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    location = vehicle.get_location()
    log = f"[Collision] Time: {timestamp} | With: {actor_type} | Location: ({location.x:.1f}, {location.y:.1f}, {location.z:.1f})"
    collision_log.append(log)
    print(log)
...
video_path = os.path.join(recording_dir, "drive_output.mp4")
fourcc = cv2.VideoWriter_fourcc(*'mp4v')
video_writer = cv2.VideoWriter(video_path, fourcc, 30.0, (800, 600))
...
pygame.display.set_caption("CARLA Manual Drive (Logitech G920) - Press Q to Quit")
...
try:
    while running:
        ...
        overlay = font.render(f"Gear: {gear_text} | Speed: {speed_kmh:.1f} km/h", True, (255, 255, 255))
    ...
    import csv
csv_path = os.path.join(recording_dir, "collision_log.csv")
with open(csv_path, 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["Timestamp", "Object Hit", "X", "Y", "Z"])
    for log in collision_log:
        try:
            parts = log.split('|')
            timestamp = parts[0].split('Time: ')[1].strip()
            obj = parts[1].split('With: ')[1].strip()
            location = parts[2].split('Location: ')[1].strip('()')
            x, y, z = [val.strip() for val in location.split(',')]
            writer.writerow([timestamp, obj, x, y, z])
        except Exception as e:
            print(f"Error parsing log: {log} => {e}")

print("--- COLLISION LOG REPORT ---")
